...
CREATE TABLE material_stock_flow (
    ...
    flow_type TINYINT NOT NULL COMMENT '类型：1-入库，2-领料，3-退料，4-盘点，5-次品处理',
...
CREATE TABLE work_order_material_loss (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT 'ID',
    work_order_id BIGINT NOT NULL COMMENT '工单ID',
    material_id BIGINT NOT NULL COMMENT '原料ID',
    batch_id BIGINT COMMENT '批次ID',
    plan_quantity DECIMAL(10,2) DEFAULT 0 COMMENT '计划用量',
    actual_used_quantity DECIMAL(10,2) DEFAULT 0 COMMENT '实际使用量',
    ...
    loss_type TINYINT COMMENT '损耗类型：1-加工损耗，2-质检损耗，3-其他损耗',
    operator_id BIGINT COMMENT '操作人ID',
    operator_name VARCHAR(50) COMMENT '操作人姓名',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    ...
    INDEX idx_work_order_id (work_order_id),
    INDEX idx_material_id (material_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='工单原料损耗表';
...
INSERT INTO material (material_name, material_code, material_type, specification, unit, quantity, available_quantity, warning_quantity, unit_price, status, need_refrigeration, shelf_life_days, supplier, origin) VALUES
('腰果仁', 'YGR001', 1, 'A级', 'kg', 500, 500, 100, 50.00, 1, 0, 180, '越南坚果供应商', '越南'),
('杏仁', 'XR001', 1, '特级', 'kg', 300, 300, 80, 45.00, 1, 0, 180, '美国杏仁进口商', '美国加州'),
('新鲜牛肉', 'XN001', 2, '牛后腿肉', 'kg', 200, 200, 50, 80.00, 1, 1, 7, '本地肉类加工厂', '内蒙古'),
('新鲜猪肉', 'XZ001', 2, '猪瘦肉', 'kg', 150, 150, 40, 35.00, 1, 1, 7, '本地肉类加工厂', '河南'),
('芒果', 'MG001', 3, '台农芒', 'kg', 400, 400, 100, 15.00, 1, 1, 14, '海南水果基地', '海南'),
('白砂糖', 'BST001', 4, '一级', 'kg', 1000, 1000, 200, 8.00, 1, 0, 365, '本地糖厂', '广西'),
('食用盐', 'SYY001', 4, '精制盐', 'kg', 500, 500, 100, 3.00, 1, 0, 730, '盐业公司', '江苏'),
('辣椒粉', 'LJF001', 4, '特辣', 'kg', 100, 100, 20, 25.00, 1, 0, 180, '四川调料厂', '四川');
